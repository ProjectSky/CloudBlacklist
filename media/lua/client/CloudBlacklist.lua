...
CloudBlacklist.OnTick = function(tick)
    if tick <= 0 then
        sendClientCommand(
            ...
            'CheckPlayer',
            'OnJoinGame',
            {name = getOnlineUsername(), steamid = getCurrentUserSteamID()}
        )
    end
end
...
CloudBlacklist.OnServerCommand = function(module, command, args)
    ...
    if module ~= 'CloudBlacklistServer' then
        return
    end
    if command == 'Disconnect' then
        ...
        writeLog(
            'CloudBlacklist',
            getText('Tooltip_CloudBlacklist_trigger', getOnlineUsername(), getCurrentUserSteamID(), TIME)
        )
    ...
    elseif command == 'LoadFail' then
        ...
        writeLog(
            'CloudBlacklist',
            getText('Tooltip_CloudBlacklist_loadfail', getOnlineUsername(), getCurrentUserSteamID(), TIME)
        )
        Events.OnTick.Add(CloudBlacklist.ForceExit)
        Events.EveryTenMinutes.Add(CloudBlacklist.EveryTenMinutes)
    end
...
function ISServerDisconnectUI:createChildren()
    ...
    if not CloudBlacklist_loadfail then
        CloudBlacklist.showMSG(
            getText('UI_CloudBlacklist_BAN_MSG', getText('Tooltip_CloudBlacklist_Reason_' .. BanReason))
        )
    else
